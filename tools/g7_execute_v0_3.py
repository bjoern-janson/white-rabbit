...
def one(run, rep, task, cond):
    ...
    content=pre+"\n\n--- TARGET ---\n"+TASKS[task]
    body=json.dumps({"model":"qwen38-27b","messages":[{"role":"user","content":content}],"stream":False,"max_tokens":512},ensure_ascii=False,separators=(",",":")).encode("utf-8")
...
def main():
    ...
    dump(OUT/"execution-identity.json",{"assay_commit":"00874aa34d2d0a2d4644765bd4e89a293d12d01a","manifest_commit":None,"started_at":utc(),"slots":30})
    for row in ORDER: one(*row)
    print("ORIGINAL_30_SLOT_V03_PASS_COMPLETE",flush=True)
