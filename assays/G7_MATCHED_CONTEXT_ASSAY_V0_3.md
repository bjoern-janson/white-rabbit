# G7 Matched-Context Assay v0.3

Version: `G7_MATCHED_CONTEXT_ASSAY_V0.3`

Status: `CONSTITUTED / NOT_EXECUTED / REVIEW_REQUIRED / NON_AUTHORIZING`

Constitution starting authority: `e02b8b97d77efac5134502c9207e6fecfb2ca318`
...
| Budget calibration | `assays/G7_GENERATION_BUDGET_CALIBRATION_V0_1.md` | `23cd33dad0fad7e91d1a9ebe06e7cf0f28c33c99` | parameter authority only |
| Budget calibration result | `assays/G7_GENERATION_BUDGET_CALIBRATION_V0_1_RESULT.md` | `e02b8b97d77efac5134502c9207e6fecfb2ca318` | instrument evidence only |
...
v0.2 remains valid historical evidence:

```text
G7 v0.2, max_tokens=64
    -> CONTROL_ADEQUACY_FAIL
```

The separate B*-only generation-budget calibration then established:

```text
15/15 admissible
15/15 finish_reason=stop
Q1=5/5, Q2=5/5, Q3=5/5
max_tokens=512
```

Therefore the shallowest supported revision is the execution envelope only:

```text
max_tokens: 64 -> 512
```

v0.2 is not invalidated. Its observations remain historical evidence that the 64-token envelope bound baseline answer emission before completion. The calibration observations are instrument evidence only and are not v0.3 scientific samples.

```text
calibration earns parameter authority
!= calibration supplies scientific assay samples
```

v0.3 is a fresh successor assay. No historical v0.2 observation or calibration observation is reused, pooled, or counted toward its 30 observations.
...
Preserve literally `finish_reason`, `max_tokens = 512`, and explicit backend/server truncation or length fields when exposed.
...
## 23. Relation of versions

```text
v0.2:
  executed historical assay
  max_tokens=64
  CONTROL_ADEQUACY_FAIL

budget calibration v0.1:
  B*-only instrument calibration
  max_tokens=512
  GENERATION_BUDGET_CALIBRATION_PASS

v0.3:
  fresh successor matched-context assay
  max_tokens=512
  all other substantive design dimensions inherited from v0.2
```
...
```text
G7_V0_3_CONSTITUTED / NOT_EXECUTED / REVIEW_REQUIRED
```
...
```text
artifact: G7_MATCHED_CONTEXT_ASSAY_V0.3
status: CONSTITUTED / NOT_EXECUTED / REVIEW_REQUIRED / NON_AUTHORIZING
Theta_G7: effect of C_improve content under matched instructional form
conditions: B* and C
tasks: 3
